...
def calculate_feature_importance(df_info, feature_names, feature_value, no_donation_columns, skewed_target_value,
    ...
    if (no_donation_columns or is_similar_file == True):
        ...
        pdf.multi_cell(h=5.0, w=0,
                       txt="Note: Feature Importance is based on the testing set of the similar donor file stored in Predict Me's server."
...
def print_confusion_matrix_classification_report(y_test, y_pred, no_donation_columns, skewed_target_value,
    ...
    if (no_donation_columns or is_similar_file == True):
        pdf.multi_cell(h=5.0, w=0, align='L',
                       txt="Note: Confusion Matrix Plot is based on the testing set of the similar donor file stored in Predict Me's server.")
    ...
    pdf.multi_cell(h=5.0, w=0, align='L',
                   txt="Based on the confusion matrix, a total of {} ({} + {}) samples belong to class donor and {} ({} + {}) samples belong to class non-donor. "
...
def plot_roc_curve(roc_fpr, roc_tpr, roc_auc, top_5_models, no_donation_columns, skewed_target_value, is_similar_file):
    ...
    pdf.ln(2)
    pdf.multi_cell(h=5.0, w=0,
                   txt="A model with high accuracy is represented by a line that travels from the bottom left of the plot to the top left "
    ...
    if (no_donation_columns or is_similar_file == True):
        pdf.multi_cell(h=5.0, w=0,
                       txt="Note: ROC is based on the testing set of the similar donor file stored in Predict Me's server.")
        pdf.ln(1)
...
def model_selection(X, y, X_pred, donation_columns, cat_col, donor_df, no_donation_columns, skewed_target_value,
    ...
    feature_importance_dict['ensemble'] = np.mean(np.vstack(
        (np.vstack((feature_importance_dict.get(lst_models[0][0]), feature_importance_dict.get(lst_models[1][0]))),
    ...
    if (not no_donation_columns) & (not is_similar_file) & (skewed_target_value == False):
        ...
        model_output_data['training'] = convert_number_format(X_train.shape[0])
        pdf.ln(0.5)
        pdf.multi_cell(h=5.0, w=0, txt="              b. 20% ({}) of data used for testing the model".format(
            convert_number_format(X_test.shape[0])))
        model_output_data['testing'] = convert_number_format(X_test.shape[0])
        pdf.ln(0.5)
    if ((skewed_target_value == True and is_similar_file == False) or (
            skewed_target_value_similar == True and is_similar_file == True)):
        pdf.ln(1)
    ...
    # save donation columns
    cprint(f"Model donation_columns --> {donation_columns}", 'cyan')
    ...
    if (no_donation_columns or is_similar_file == True):
        ...
        pdf.ln(0.25)
        pdf.multi_cell(h=5.0, w=0,
                       txt="         donor or class non-donor 'data imbalance') to list donation column(s). In such cases, to execute the model")

        pdf.ln(0.25)
        pdf.multi_cell(h=5.0, w=0,
                       txt="         successfully, data features from the input file are matched against data features on the similar donor files stored in")

        pdf.ln(0.25)
        pdf.multi_cell(h=5.0, w=0,
                       txt="         the Predict Me's server. Donor file with the highest match rate is selected and processed using Natural Language")
    ...
    if (len(cat_col) != 0 and is_similar_file == False):
        ...
        pdf.ln(0.25)
        pdf.multi_cell(h=5.0, w=0,
                       txt="          donation information OR only has one class to list categorical data features. In such cases, the donor file")

        pdf.ln(0.25)
        pdf.multi_cell(h=5.0, w=0,
                       txt="          with the highest match rate is selected to calculate data features importance.")
    ...
    pdf.ln(1)
    pdf.multi_cell(h=5.0, w=0,
                   txt="     1. Text Data Conversion: Process of converting text data into a form that a model can understand.")
    pdf.ln(0.5)

    if (no_donation_columns == True or is_similar_file == True):

        pdf.multi_cell(h=5.0, w=0,
                       txt="     2. Target Value: Total donation columns from the input file are calculated to assign Target Values. For example, in ")

        pdf.ln(0.25)
        pdf.multi_cell(h=5.0, w=0,
                       txt="          each row, if 50% of the total donation columns have a donation amount >= 1, the model assigns that row as 1")

        pdf.ln(0.25)
        pdf.multi_cell(h=5.0, w=0,
                       txt="          otherwise 0. Note: The input data is missing donation information OR only has one class. In such cases,")

        pdf.ln(0.25)
        pdf.multi_cell(h=5.0, w=0,
                       txt="          the donor file with the highest match rate is selected to calculate Target Value.")
    ...
    pdf.multi_cell(h=5.0, w=0,
                   txt="     3. Assign Target Value/Class (1 and 0): 1 = class donor or 1. 0 = class non-donor or 0.")
    pdf.ln(0.5)
    ...
    pdf.multi_cell(h=5.0, w=0,
                   txt="     6. Data Feature Importance: Process of selecting data features that contributes the most in prediction.")
    pdf.ln(0.5)

    pdf.multi_cell(h=5.0, w=0,
                   txt="     7. Classifier/Model: Classifier or classification model is an algorithm that predicts classes.")
    pdf.ln(0.5)

    pdf.multi_cell(h=5.0, w=0,
                   txt="     8. Ensemble Method: Technique of combining several models to generate one best model.")
    pdf.ln(0.5)

    pdf.multi_cell(h=5.0, w=0,
                   txt="     9. Soft Voting: Process of generating best result by averaging all the predicted probabilities calculated by distinct ")
    ...
    pdf.multi_cell(h=5.0, w=0,
                   txt="     11. Precision: Fraction of correct predictions for a certain class. It refers to the percentage "
                       "of results that are relevant.")
    pdf.ln(0.5)

    pdf.multi_cell(h=5.0, w=0,
                   txt="     12. Recall: Fraction of correct predictions of all actual classes. It refers to the percentage of total relevant results")
    ...
    pdf.multi_cell(h=5.0, w=0,
                   txt="     13. F1- Score: Measure of a model's accuracy. A perfect model has an F1-score of 1.")
    pdf.ln(0.5)

    pdf.multi_cell(h=5.0, w=0,
                   txt="     14. Confusion Matrix Plot: Visualized table describing the performance of a model. Each row in a confusion matrix")
    pdf.ln(0.25)
    pdf.multi_cell(h=5.0, w=0,
                   txt="           represents an actual class, while each column represents a predicted class (or vice versa). Classification ")
    pdf.ln(0.25)
    ...
    pdf.multi_cell(h=5.0, w=0,
                   txt="     16. Threshold: Cut-off value on a probability score to separate a donor from a non-donor. ")
    pdf.ln(0.5)

    pdf.multi_cell(h=5.0, w=0,
                   txt="     17. Probability Score: Predicted probability (likelihood) score of an individual to donate.")
    pdf.ln(0.5)

    pdf.multi_cell(h=5.0, w=0,
                   txt="     18. Predicted Classification (1 and 0): 1 indicates an individual likely to donate. 0 indicates "
...
def generate_prediction_file(df, model_f1_score, classification_full_pred, classification_full_pred_prob, y,
    ...
    if (no_donations_columns == True or is_similar_file == True):
        pdf.multi_cell(h=5.0, w=0, align='L',
                       txt="Soft voting ensemble method used to combine the predictions of the top 3 best fit classifiers (models)."
    ...
    for ind, m in enumerate(top_5_model):
        ...
        if (no_donations_columns == True or is_similar_file == True):
            ...
            pdf.ln(0.75)
            pdf.multi_cell(h=5.0, w=0, txt="        c. Donors Predicted: {}% ({} out of {})".format(donor_per,
            ...
            pdf.multi_cell(h=5.0, w=0, txt="        a. Assigned Target Value (class donor): {}".format(
                convert_number_format(y.count(1))))
            pdf.ln(0.75)
            ...
            pdf.ln(0.75)
            pdf.multi_cell(h=5.0, w=0, txt="        d. Donors Predicted: {}% ({} out of {})".format(donor_per,
                                                                                                    convert_number_format(
                                                                                                        donor_count),
                                                                                                    convert_number_format(
                                                                                                        df.shape[0])))
        ...
        if (no_donations_columns == False and len(y) != 0 and is_similar_file == False):
            ...
            print_confusion_matrix_classification_report(y, df[prediction_column_name], no_donations_columns,
                                                         skewed_target_value, is_similar_file)

            # calc roc_fpr, roc_tpr, roc_auc for the real input file
            fpr_inputfile, tpr_inputfile, auc_inputfile = calculate_fpr_tpr(None, y, df[prediction_column_name], None,
                                                                            df[probability_column_name])
            plot_roc_curve(fpr_inputfile, tpr_inputfile, auc_inputfile, top_5_model, no_donations_columns,
                           skewed_target_value, is_similar_file)
    ...
    for i in range(threshold):
        ...
        if (donation_columns != "[]"):
            donation_columns_df = process_donation_columns(df, donation_columns, no_donations_columns,
                                                           skewed_target_value)
            no_donations_columns_new, skewed_target_value_new = check_skew_donation_cols(donation_columns_df,
                                                                                         donation_columns)
            if skewed_target_value_new == False:
                print("The most similar file to the input file is {}: ".format(x[i]))
                return df, donation_columns, donation_columns_df, skewed_target_value_new, x[i].split('\\')[-1], \
                       common_features[x[i]]
...
def print_steps_taken(is_similar_file):
    ...
    if (is_similar_file == True):
        pdf.multi_cell(h=5.0, w=0,
                       txt="     4. Assign Target Value. Target values are the predicted variable. Since the uploaded donor file is missing donation")
    ...
    if ((skewed_target_value == True and is_similar_file == False) or (
        ...
        pdf.ln(0.5)
        pdf.multi_cell(h=5.0, w=0,
                       txt="     9. Evaluate performance metrics of 10 classifiers (models) and select top 3 best fit classifiers.")
        pdf.ln(0.5)
        pdf.multi_cell(h=5.0, w=0,
                       txt="     10. Combine top 3 best fit classifiers predictions using a soft voting ensemble method.")
        pdf.ln(0.5)
        pdf.multi_cell(h=5.0, w=0,
                       txt="     11. Create Confusion Matrix, Classification Report and Receiver Operating Characteristic (ROC) Curve.")
        pdf.ln(0.5)
        ...
        pdf.ln(0.5)
        pdf.multi_cell(h=5.0, w=0,
                       txt="     8. Evaluate performance metrics of 10 classifiers (models) and select top 3 best fit classifiers.")
        pdf.ln(0.5)
        pdf.multi_cell(h=5.0, w=0,
                       txt="     9. Combine top 3 best fit classifiers predictions using a soft voting ensemble method.")
        pdf.ln(0.5)
        pdf.multi_cell(h=5.0, w=0,
                       txt="     10. Create Confusion Matrix, Classification Report and Receiver Operating Characteristic (ROC) Curve.")
        pdf.ln(0.5)
    ...
    if (is_similar_file == True):
        pdf.multi_cell(h=5.0, w=0,
                       txt="     12. Generate model summary report (PDF) and CSV file with Donor Probability Score and Donor Predicted ")

        pdf.ln(0.25)
        pdf.multi_cell(h=5.0, w=0, txt="           Classification columns appended back to the processed file.")
    else:
        pdf.multi_cell(h=5.0, w=0,
                       txt="     13. Generate model summary report (PDF) and CSV file with the Assigned Target Value, Donor Probability Score ")

        pdf.ln(0.25)
        pdf.multi_cell(h=5.0, w=0,
                       txt="           and Donor Predicted Classification columns appended back to the processed file.")
...
def run_model(data_file_path, donation_cols, text_cols, send_obj):
    ...
    if (len(donation_columns) != 0):
        ...
        if (no_donations_columns == True):
            df, donation_columns_similar, donation_columns_df_similar, skewed_target_value_similar, similar_filename, common_features = find_similar_files(
                file_path, no_donations_columns, skewed_target_value)
            if (len(df) == 0):
                raise ValueError(
                    "The similar file is not found, and the donation columns of input file does not exist. "
    ...
    if (is_similar_file == True and similar_filename != None and common_features != None):
        print("The most similar file to the input file is: {} \n"
              ...
              "Categorical columns of the similar file used for prediction models are: {}".format(similar_filename,
                                                                                                  common_features,
                                                                                                  ", ".join(
                                                                                                      df_info.columns[
                                                                                                      0:-1])))
    ...
    df_final, best_model = generate_prediction_file(donor_df, model_f1_score, classification_full_pred,
                                                    classification_full_pred_prob, y, feature_importance_dict, roc_fpr,
                                                    roc_tpr,
                                                    roc_auc, y_test_dict, y_pred_dict, feature_names, df_info,
                                                    donation_columns_df,
                                                    no_donations_columns, skewed_target_value,
    ...
    MODEL_FILES_PATH['PDF_FILE'] = "{}/{}_{}_{}_report.pdf".format(pdf_report_path, file_path.split(".")[0], best_model,
                                                                   today_date)
    df_final.to_csv(
        "{}/{}_{}_{}_prediction.csv".format(prediction_path, file_path.split(".")[0], best_model, today_date),
        index=None)
    MODEL_FILES_PATH['CSV_FILE'] = "{}/{}_{}_{}_prediction.csv".format(prediction_path, file_path.split(".")[0],
                                                                       best_model,
                                                                       today_date)
